...
func SendHandoverRequest(sourceUe *context.RanUe, targetRan *context.AmfRan, cause ngapType.Cause,
	...
	// Patch embedded handover state-sync (if present) to match the AMF UE NGAP ID used towards the target RAN.
	// This is needed when the AMF allocates a new AMF UE NGAP ID for the target RanUe during HO (free5GC behavior),
	// but the embedded transfer was generated by the source RAN/N3IWF with its own AMF UE NGAP ID.
	if targetUe != nil && len(sourceToTargetTransparentContainer.Value) > 0 {
		var stt ngapType.SourceNGRANNodeToTargetNGRANNodeTransparentContainer
		if err := aper.UnmarshalWithParams(sourceToTargetTransparentContainer.Value, &stt, "valueExt"); err != nil {
			sourceUe.Log.Debugf("Handover state-sync: decode SourceToTarget container failed: %v", err)
		} else if raw := stt.RRCContainer.Value; len(raw) > 0 && raw[0] == '{' {
			var obj map[string]json.RawMessage
			if err := json.Unmarshal(raw, &obj); err != nil {
				sourceUe.Log.Warnf("Handover state-sync: parse embedded transfer failed: %v", err)
			} else {
				obj["amfUeNgapId"] = json.RawMessage([]byte(strconv.FormatInt(targetUe.AmfUeNgapId, 10)))
				if newRaw, err := json.Marshal(obj); err != nil {
					sourceUe.Log.Warnf("Handover state-sync: marshal patched transfer failed: %v", err)
				} else {
					stt.RRCContainer.Value = newRaw
					if patched, err := aper.MarshalWithParams(stt, "valueExt"); err != nil {
						sourceUe.Log.Warnf("Handover state-sync: encode patched SourceToTarget container failed: %v", err)
					} else {
						sourceToTargetTransparentContainer.Value = patched
						sourceUe.Log.Debugf("Handover state-sync: patched embedded transfer AMF UE NGAP ID to %d", targetUe.AmfUeNgapId)
					}
				}
			}
		}
