...
func cloneGlobalRanNodeID(src *models.GlobalRanNodeId) *models.GlobalRanNodeId {
	if src == nil {
		return nil
	}
	clone := &models.GlobalRanNodeId{
		N3IwfId: src.N3IwfId,
		NgeNbId: src.NgeNbId,
		WagfId:  src.WagfId,
		TngfId:  src.TngfId,
...
func targetAccessType(target *models.NgRanTargetId) models.AccessType {
	if target == nil || target.RanNodeId == nil {
		return ""
	}
	ran := target.RanNodeId
	switch {
	case ran.N3IwfId != "":
		return models.AccessType_NON_3_GPP_ACCESS
	case ran.WagfId != "" || ran.TngfId != "" || ran.TwifId != "":
		return models.AccessType_NON_3_GPP_ACCESS
	default:
...
func HandleHandoverRequiredTransfer(b []byte, ctx *SMContext, updateData *models.SmfPduSessionSmContextUpdateData) error {
	...
	if updateData != nil {
		if access := targetAccessType(updateData.TargetId); access != "" {
			ctx.TargetAccessType = access
		} else {
			ctx.TargetAccessType = ctx.AnType
		}
		if updateData.TargetId != nil && updateData.TargetId.RanNodeId != nil {
			ctx.TargetRanNodeID = cloneGlobalRanNodeID(updateData.TargetId.RanNodeId)
		} else {
			ctx.TargetRanNodeID = nil
		}
	}
