...
static int bind_local_if_needed(int fd, const char* local_ip) {
    if (!local_ip || !*local_ip) return 0; // nessun bind richiesto
    sockaddr_in local{}; local.sin_family = AF_INET; local.sin_port = htons(0); // FIXED PORT 0
...
int main(int argc, char* argv[]) {
    if (argc < 3 || argc > 4) {
        std::cerr << "Usage: " << argv[0] << " <server_ip> <server_port> [local_ip]\n";
        return 1;
    }
    const char* server_ip = argv[1];
    int server_port = std::stoi(argv[2]);
    const char* local_ip  = (argc == 4 ? argv[3] : nullptr);
    ...
    if (bind_local_if_needed(fd, local_ip) < 0) { close(fd); return 1; }

    // 4) prepara peer
    sockaddr_in peer{}; peer.sin_family = AF_INET; peer.sin_port = htons(server_port);
    if (inet_pton(AF_INET, server_ip, &peer.sin_addr) != 1) {
        perror("inet_pton(server)"); close(fd); return 1;
    }

    std::cerr << "[SCTP] Connecting to " << server_ip << ":" << server_port;
    if (local_ip && *local_ip) std::cerr << "  (from " << local_ip << ")";
    std::cerr << " ...\n";
