...
func (s *Server) HandleUEContextReleaseCommand(
	...
	// Cancel any active HO DL buffers for this UE before releasing context.
	if hoBuffer := s.Context().HOBuffer; hoBuffer != nil {
		for _, pdu := range shared.PduSessionList {
			if pdu != nil && pdu.GTPConnInfo != nil {
				hoBuffer.Cancel(pdu.GTPConnInfo.IncomingTEID)
			}
		}
	}
...
func (s *Server) HandleHandoverRequest(
	...
	// Activate DL handover buffers for all admitted PDU sessions so that
	// packets arriving from the UPF (after the early path switch) are held
	// until the UE completes MOBIKE and the XFRM rules are reinstalled with
	// the correct peer IP.
	if hoBuffer := n3iwfCtx.HOBuffer; hoBuffer != nil {
		for _, item := range admittedItems {
			pduID := int64(item.PDUSessionID)
			if pdu := sharedCtx.FindPDUSession(pduID); pdu != nil && pdu.GTPConnInfo != nil {
				hoBuffer.Activate(pdu.GTPConnInfo.IncomingTEID)
			}
		}
	}
